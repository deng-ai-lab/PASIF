...
class TargetDiff(BaseDiff):
    ...
    def finetune(self, batch, sampler):
        ...
        t = self.sample_time(B, device = x_lig_0.device).clamp(min=1, max=self.num_diffusion_timesteps)
        t_minus = (t - 1).clamp(min=0, max=self.num_diffusion_timesteps-1)
        unif_probs = torch.ones_like(c_lig_0) / c_lig_0.shape[-1]
        gauss_noise = torch.randn_like(x_lig_0)

        # x_lig_unknown, _ = self.pos_scheduler.forward_add_noise(x_lig_0, t, batch_idx_lig, torch.ones_like(gen_flag_lig))
        # x_lig_former, _ = self.pos_scheduler.forward_add_noise(x_lig_0, t_minus, batch_idx_lig, torch.ones_like(gen_flag_lig))
        # pt0 = self.type_scheduler.Kt.to(x_lig_0.device)
        # pt0_minus = pt0[t_minus][batch_idx_lig]
        # pt0 = pt0[t][batch_idx_lig]
        # c_lig_unknown = torch.einsum('li, lij->lj', c_lig_0, pt0)
        # c_lig_former = torch.einsum('li, lij->lj', c_lig_0, pt0_minus)
        
        gamma = t / self.num_diffusion_timesteps
        x_lig_unknown = (1 - gamma[batch_idx_lig][:, None]) * x_lig_0 + gamma[batch_idx_lig][:, None] * gauss_noise
        log_c_lig_unknown = (1 - gamma[batch_idx_lig][:, None]) * torch.log((c_lig_0.clamp(min=0.001, max=0.999)))
        c_lig_unknown = F.softmax(log_c_lig_unknown, dim=-1)

        # x_lig_in = gen_flag_lig[:, None].float() * x_lig_unknown + \
        #            (1. - gen_flag_lig[:, None].float()) * x_lig_0
        # c_lig_in = gen_flag_lig[:, None].float() * c_lig_unknown + \
        #            (1. - gen_flag_lig[:, None].float()) * c_lig_0
        x_lig_in = x_lig_unknown
        c_lig_in = c_lig_unknown
        # x_lig_tar = gen_flag_lig[:, None].float() * x_lig_former + \
        #            (1. - gen_flag_lig[:, None].float()) * x_lig_0
        # c_lig_tar = gen_flag_lig[:, None].float() * c_lig_former + \
        #            (1. - gen_flag_lig[:, None].float()) * c_lig_0
        kl_grad = torch.log(c_lig_in.clamp(min=0.001, max=0.999) / c_lig_0.clamp(min=0.001, max=0.999))
        # F_mat = torch.diag_embed(c_lig_in) - c_lig_in.unsqueeze(2) @ c_lig_in.unsqueeze(1)  # (L, S, S)
        # F_mat_inv = torch.inverse(F_mat+ 0.001 * torch.eye(c_lig_in.shape[-1]).unsqueeze(0).to(c_lig_in.device))
        # nat_grad = torch.einsum('nij, nj->ni', F_mat_inv, kl_grad)
        # jac_mat = self.softmax_jacobian(c_lig_in, temp)
        c_tar = c_lig_in/gamma[batch_idx_lig][:, None] * (kl_grad - (c_lig_in*kl_grad).sum(-1, keepdim=True))
        x_tar = 1/gamma[batch_idx_lig][:, None] * (x_lig_in - x_lig_0)
        
        # c_lig_tar = c_lig_tar.clamp(min=0.001, max=0.999)
        x_lig_mu = x_lig_in
        c_lig_mu = c_lig_in
        c_lig_mu_norm = c_lig_mu
        from torch.distributions import Categorical
        dist = Categorical(probs=c_lig_mu_norm)
        v_lig_mu = dist.sample()
        
        x_lig_t, _ = self.pos_scheduler.forward_add_noise(x_lig_mu, t, batch_idx_lig, torch.ones_like(gen_flag_lig))
        c_lig_t, v_lig_t = self.type_scheduler.forward_add_noise(v_lig_mu, t, batch_idx_lig, torch.ones_like(gen_flag_lig))

        x_lig_emb, x_rec_emb, h_lig_emb, h_rec_emb = self.context_embedder(x_lig_t, x_rec_0, c_lig_t, v_rec_0, aa_rec_0, 
                                                                batch_idx_lig, batch_idx_rec, lig_flag, rec_flag, t)
        
        context_composed, batch_idx, _ = compose_context({'x': x_lig_emb, 'h': h_lig_emb, 'gen_flag': torch.ones_like(gen_flag_lig), 'lig_flag': lig_flag},
                                                        {'x': x_rec_emb, 'h': h_rec_emb, 'gen_flag': gen_flag_rec, 'lig_flag': rec_flag},
                                                        batch_idx_lig, batch_idx_rec)
        ...
        grad_x_obs = 0.5 * h_mask * (x_lig_mu - x_lig_0)
        ...
        v_lig_mu_onehot = F.one_hot(v_lig_mu, num_classes = self.num_classes).float()
        scale_in = torch.concat([grad_c_obs, grad_c_noise, c_lig_pred, v_lig_mu_onehot, t_in], dim=-1)
        vec_in = torch.concat([x_lig_mu[:, None, :], grad_x_obs[:, None, :], grad_x_noise[:, None, :], x_lig_out[:, None, :]], dim=1)
        x_in = x_lig_mu
        ...
        # grad_x = grad_x_obs + 0.25*grad_x_noise + grad_x_hat
        # grad_c = grad_c_obs + grad_c_noise + grad_c_hat
        grad_x = grad_x_hat
        grad_c = c_lig_mu * (grad_c_hat - (c_lig_mu*grad_c_hat).sum(-1, keepdim=True))
        grad_c_true = torch.matmul(jac_mat.transpose(-1, -2), grad_c[:, :, None])[:, :, 0]
        x_pred = x_lig_mu - 0.001 * grad_x
        c_pred = F.softmax((c_lig_mu - 0.001 * grad_c_true)/temp, dim=-1).clamp(min=0.001, max=0.999)

        # loss_x = ((x_pred - x_lig_tar)**2).sum(-1).mean()
        # loss_c = F.kl_div(c_lig_tar.log(), c_pred, reduction="batchmean")
        loss_x = ((grad_x - x_tar)**2).sum(-1).mean()
        loss_c = ((grad_c - c_tar)**2).sum(-1).mean()
        loss_reg_c = torch.norm(grad_c_hat, dim=-1).mean()
        loss_reg_x = torch.norm(grad_x_hat, dim=-1).mean()

        loss_dict = {'pos': loss_x, 'type': loss_c, 'reg_c': loss_reg_c, 'reg_x': loss_reg_x}
        results = {}
        pos_info = {'x0': x_lig_0, 'xt': x_lig_t, 'x_pred': x_pred, 'mask_gen': gen_flag_lig}
        atom_info = {'v0': v_lig_0, 'vt': v_lig_t, 'c_pred': c_pred, 'mask_gen': gen_flag_lig}
        results.update(pos_info)
        results.update(atom_info)
