...
class CTNVPScheduler(VPScheduler):
    ...
    def forward_add_noise(self, x, t, batch_idx, gen_flag, noise=None, zero_center=False):
        ...
        if t[0].item() == self.alphas_cumprod.shape[0]:
            a = self.alphas_cumprod.index_select(0, t-1) * self.alphas.index_select(0, t-1)
    ...
    def multi_step_add_noise(self, xs, t, s, batch_idx, gen_flag, noise=None, zero_center=False):
        mask_generate = gen_flag
        ...
        if zero_center:
            com_noise = scatter_mean(noise, batch_idx, dim=0)[batch_idx]
            pos_noise = noise - scatter_mean(noise, batch_idx, dim=0)[batch_idx]
        if t[0].item() == self.alphas.shape[0]:
            alpha_t = self.alphas_cumprod.index_select(0, t-1) * self.alphas.index_select(0, t-1)
            alpha_t = alpha_t[batch_idx].unsqueeze(-1)
        else:
            alpha_t = self.alphas_cumprod.index_select(0, t)[batch_idx].unsqueeze(-1)
        alpha_s = self.alphas_cumprod.index_select(0, s)[batch_idx].unsqueeze(-1)
        alpha_ts = alpha_t / alpha_s
        x_t = alpha_ts.sqrt() * xs + (1. - alpha_ts).sqrt() * noise
        if zero_center:
            return torch.where(mask_generate.unsqueeze(-1), x_t, xs), pos_noise, com_noise
        return torch.where(mask_generate.unsqueeze(-1), x_t, xs), noise
    
    def get_x_hat(self, xt, eps, t, batch_idx, gen_flag):
        ...
        if t[0].item() == self.alphas.shape[0]:
            ...
            alpha_t = self.alphas_cumprod.index_select(0, t)[batch_idx].unsqueeze(-1)
            sigma_t = self.sqrt_one_minus_alphas_cumprod.index_select(0, t)[batch_idx].unsqueeze(-1)
        x_hat = 1/alpha_t * xt - sigma_t / alpha_t * torch.randn_like(xt)
    ...
    def sample_xs_given_xt(self, eps_t, xt, t, s, batch, gen_flag):
        # Compute the mean of the diffusion posterior q(x_s | x_t, x_0)
        if t[0].item() == self.alphas.shape[0]:
            alpha_t = self.sqrt_alphas_cumprod.index_select(0, t-1) * self.alphas.index_select(0, t-1).sqrt()
            alpha_t = alpha_t[batch][:,None]
            sigma_t = (1. - alpha_t**2).sqrt()
        else:
            alpha_t = self.sqrt_alphas_cumprod.index_select(0, t)[batch][:,None]
            sigma_t = self.sqrt_one_minus_alphas_cumprod.index_select(0, t)[batch][:,None]
        alpha_s = self.sqrt_alphas_cumprod.index_select(0, s)[batch][:,None]
        sigma_s = self.sqrt_one_minus_alphas_cumprod.index_select(0, s)[batch][:,None]
        alpha_ts = alpha_t / alpha_s
        sigma_ts = (sigma_t**2 - alpha_ts**2 * sigma_s**2).sqrt()
        xs_mean = xt / alpha_ts - sigma_ts**2 / alpha_ts / sigma_t * eps_t
        xs = xs_mean + sigma_ts*sigma_s/sigma_t * torch.randn_like(xs_mean)

        xs = torch.where(gen_flag[:, None], xs, xt)

        return xs

    def sample_xs_given_x0_xt(self, x0, xt, t, s, batch, gen_flag):
        # Compute the mean of the diffusion posterior q(x_s | x_t, x_0)
        if t[0].item() == self.alphas.shape[0]:
            alpha_t = self.sqrt_alphas_cumprod.index_select(0, t-1) * self.alphas.index_select(0, t-1).sqrt()
            alpha_t = alpha_t[batch][:,None]
        ...
        alpha_ts = alpha_t / alpha_s
        sigma_ts = (sigma_t**2 - alpha_ts**2 * sigma_s**2).sqrt()
        xs_mean = alpha_ts * (sigma_s/sigma_t)**2 * xt + alpha_s*sigma_ts**2/sigma_t**2 * x0
        xs = xs_mean + sigma_ts*sigma_s/sigma_t * torch.randn_like(xs_mean)
...
class TypeVPScheduler(VPScheduler):
    ...
    def __init__(self, num_timestep, num_classes, beta_start=1e-7, 
        ...
        self.init_rate_and_Kt(np.exp(log_alphas_cumprod_v), num_timestep)

    def init_rate_and_Kt(self, alphas_cumprod_v, num_timestep):
        U = np.ones((self.num_classes, self.num_classes))/self.num_classes
        I = np.eye(self.num_classes)
        alphas_cumprod_v_dt = (alphas_cumprod_v[1:]-alphas_cumprod_v[:-1])*num_timestep
        alphas_cumprod_v_dt = np.concatenate((alphas_cumprod_v_dt, alphas_cumprod_v_dt[-1:]), axis=0)
        # scale_t = alphas_cumprod_v_dt/(1 - alphas_cumprod_v)
        scale_t = - alphas_cumprod_v_dt/alphas_cumprod_v
        rate = scale_t[:, None, None] * (U - I)[None, :, :]
        Kt = alphas_cumprod_v[:, None, None] * I[None, ...] + (1 - alphas_cumprod_v)[:, None, None] * U[None, ...]
        self.rate = torch.from_numpy(rate).float()  # TxSxS
        self.Kt = torch.from_numpy(Kt).float()      # TxSxS

    def one_step_add_noise(self, c_lig, s, batch_idx, gen_flag):
        
        S = c_lig.shape[-1]
        alpha = self.alphas.index_select(0, s+1)
        alpha_expand = alpha[batch_idx].unsqueeze(-1)
        prob_lig_noisy = alpha_expand * c_lig + (1. - alpha_expand) * 1/S
        from torch.distributions import Categorical
        dist = Categorical(probs=prob_lig_noisy)
        v_lig_noisy = dist.sample()
        c_lig_noisy = F.one_hot(v_lig_noisy, num_classes=S).float()

        c_lig_noisy = torch.where(gen_flag.unsqueeze(-1), c_lig_noisy, c_lig)
        v_lig_noisy = c_lig_noisy.argmax(-1)

        return c_lig_noisy, v_lig_noisy

    def multi_step_add_noise(self, c_lig, t, s, batch_idx, gen_flag):

        S = c_lig.shape[-1]
        if t[0].item() == self.Kt.shape[0]:
            alpha_t = self.alphas_cumprod.index_select(0, t-1) * self.alphas.index_select(0, t-1)
            alpha_t = alpha_t[batch_idx].unsqueeze(-1)
        else:
            alpha_t = self.alphas_cumprod.index_select(0, t)[batch_idx].unsqueeze(-1)
        alpha_s = self.alphas_cumprod.index_select(0, s)[batch_idx].unsqueeze(-1)
        alpha_ts = alpha_t / alpha_s
        prob_noisy = alpha_ts * c_lig + (1 - alpha_ts) * 1/S
        from torch.distributions import Categorical
        dist = Categorical(probs=prob_noisy)
        v_lig_noisy = dist.sample()
        c_lig_noisy = F.one_hot(v_lig_noisy, num_classes=S).float()
    ...
    def sample_xs_given_xt_x0(self, c_lig_0, c_lig_t, t, s, batch_idx, gen_flag):
        
        t = t[0]
        s = s[0]
        L, S = c_lig_0.shape

        if t.item() == self.Kt.shape[0]:
            Q_t = torch.ones_like(self.Kt[0]) / S
            alpha_t = self.alphas_cumprod.index_select(0, t-1) * self.alphas.index_select(0, t-1)
        else:
            Q_t = self.Kt[t]
            alpha_t = self.alphas_cumprod.index_select(0, t)
        Q_s = self.Kt[s]
        Q_t = Q_t[None, :, :].repeat(L, 1, 1)
        Q_s = Q_s[None, :, :].repeat(L, 1, 1)

        alpha_s = self.alphas_cumprod.index_select(0, s)
        alpha_ts = alpha_t / alpha_s
        Q_ts = alpha_ts * torch.eye(S, device=c_lig_0.device) + (1 - alpha_ts) * 1/S
        Q_ts = Q_ts[None, :, :].repeat(L, 1, 1)

        qt0 = c_lig_0[:, None, :] @ Q_t @ c_lig_t[:, :, None]  # (L, 1, 1)
        qs0 = c_lig_0[:, None, :] @ Q_s  # (L, 1, S)
        qts = (Q_ts @ c_lig_t[:, :, None]).transpose(-1, -2)  # (L, 1, S)
        qs_t0 = qts * qs0 / qt0

        from torch.distributions import Categorical
        dist = Categorical(probs=qs_t0[:, 0, :])
        v_lig_s = dist.sample()
        c_lig_s = F.one_hot(v_lig_s, num_classes=S).float()

        c_lig_s = torch.where(gen_flag.unsqueeze(-1), c_lig_s, c_lig_t)
        v_lig_s = c_lig_s.argmax(-1)

        return c_lig_s, v_lig_s
    
    def compute_pt_pt(self, pt0, p0t, x, batch_idx, eps=1e-9):
        """
        param:
            -pt0: (N, S, S)
            -p0t: (L, S)
        ...
        # reshape
        pt0_selected = pt0[batch_idx]   # (L, S, S)

        # denom: every site [s -> x[i]] 的概率 (L, S)
        denom = pt0_selected[torch.arange(L), :, x_idx] + eps  # (L, S)

        # ratio = p0t / denom
        ratio = p0t / denom  # (L, S)

        # inner_sum = ratio @ qt0_selected
        # (L, S) @ (L, S, S) → (L, S)
        inner_sum = torch.bmm(ratio.unsqueeze(1), pt0_selected).squeeze(1)  # (L, S)

        return inner_sum

    def pT_pt(self, t, p0t, x, batch_idx):
        """
        param:
            -pt0: (N, S, S)
            -pT0: (N, S, S)
        ...
        pt0 = self.Kt[t][None, :, :].repeat(L, 1, 1).to(x.device)
        pT0 = self.Kt[-1][None, :, :].repeat(L, 1, 1).to(x.device)
        pT0_x = pT0[torch.arange(L), :, x_idx]
        pt0_x = pt0[torch.arange(L), :, x_idx]
    ...
    def qt(self, q0, pt0, batch_idx):
        ...
        pt0_flatten = pt0[batch_idx]  # (L, S, S)
        qt = torch.einsum('li, lij->lj', q0, pt0_flatten)

        return qt
    
    def compute_qt_qt(self, qt, x):
        """
        param:
            -qt: (L, S)
            -x: (L, S)
        ...
        L = x.shape[0]
        x_idx = torch.argmax(x, dim=-1)
        qt_x = qt[torch.arange(L), x_idx] # (L, )
        out = qt / qt_x[:, None]  # (L, S)
    ...
    def qct_c0_pred(self, log_v0, t, batch_idx):
        # compute q(vt | v0)
        if t[0].item() == self.log_alphas_cumprod_v.shape[0]:
            log_cumprod_alpha_t = self.log_alphas_cumprod_v.index_select(0, t-1) + self.log_alphas_v.index_select(0, t-1)
            ...
            log_cumprod_alpha_t = log_cumprod_alpha_t[batch_idx].unsqueeze(-1)
        log_1_min_cumprod_alpha = (1 - log_cumprod_alpha_t.exp()).log()
...
class MaskTypeSchedule(nn.Module):
    ...
    def get_Qt_cumprob(self, alpha):

        # Qt = torch.zeors((self.num_classes, self.num_classes), device=alpha.device).float()
        # Qt[self.absorbing_state, self.absorbing_state] = 1.
        # Qt[torch.arange(Qt.size(0)) != self.absorbing_state, self.absorbing_state] = 1 - alpha
        # mask = torch.ones_like(Qt, dtype=torch.bool)
        # mask[self.absorbing_state, :] = False       # 不修改第 i 行
        # mask[:, self.absorbing_state] = False       # 不修改第 j 列
        # Qt[mask] = alpha

        Qt = torch.eye(self.num_classes, device=alpha.device) * alpha
        Qt[:, self.absorbing_state] += 1 - alpha
    ...
    def multi_step_add_noise(self, c_lig, t, s, batch_idx, gen_flag):
        ...
        alpha_t = lambda t: 1 - t/self.num_timestep
        Q_ts = self.get_Qt_cumprob(alpha_t(t) / alpha_t(s))
        qts = (c_lig[:, None, :] @ Q_ts)[:, 0, :]

        from torch.distributions import Categorical
        dist = Categorical(probs=qts)

        v_lig_t = dist.sample()
        c_lig_t = F.one_hot(v_lig_t, num_classes=S).float()

        c_lig_t = torch.where(gen_flag.unsqueeze(-1), c_lig_t, c_lig)
        v_lig_t = c_lig_t.argmax(-1)
        return c_lig_t, v_lig_t
    ...
    def sample_xs_given_xt_x0(self, c_lig_0, c_lig_t, t, s, batch_idx, gen_flag):
        ...
        alpha_t = lambda t: 1 - t/self.num_timestep
        Q_t = self.get_Qt_cumprob(alpha_t(t))
        Q_s = self.get_Qt_cumprob(alpha_t(s))
        Q_ts = self.get_Qt_cumprob(alpha_t(t) / alpha_t(s))
        ...
        qs0 = c_lig_0[:, None, :] @ Q_s  # (L, 1, S)
        qts = (Q_ts @ c_lig_t[:, :, None]).transpose(-1, -2)  # (L, 1, S)
        qs_t0 = qts.clamp(min=0.001, max=0.999) * qs0.clamp(min=0.001, max=0.999) / qt0.clamp(min=0.001, max=0.999)
    ...
    def pT_pt(self, p0t, pt0, x, batch_idx):

        L = x.shape[0]
        x_idx = torch.argmax(x, dim=-1)

        # mask_prob = t / self.num_timestep
        # pt0 = torch.eye(self.num_classes, dtype=torch.float, device=x.device)
        # pt0 = pt0 * (1. - mask_prob)
        # pt0[:, self.absorbing_state] +=  mask_prob
        # pt0 = pt0[None, :, :].repeat(L, 1, 1)
        
        mask_prob = (self.num_timestep - 1) / self.num_timestep
        pT0 = torch.eye(self.num_classes, dtype=torch.float, device=x.device)
        pT0 = pT0 * (1. - mask_prob)
        pT0[:, self.absorbing_state] +=  mask_prob
        pT0 = pT0[None, :, :].repeat(L, 1, 1)
        ...
        pT_pt = p0t * pT0_x / pt0_x.clamp(max=0.999, min=0.001)
        pT_pt = (pT_pt).sum(-1)
    ...
    def qt(self, q0, pt0, batch_idx):
        ...
        return qt.clamp(min=0.001, max=0.999)
...
class DiffsbddVariationalScheduler(VariationalScheduler):
    ...
    def remove_mean_batch(self, x_lig, x_rec, batch_idx_lig, batch_idx_rec):
        ...
        x_lig_out = x_lig - mean[batch_idx_lig]
        x_rec = x_rec - mean[batch_idx_rec]
        return x_lig_out, x_rec
    ...
    def forward_pos_center_noise(self, x, t, batch_idx, gen_flag, noise=None, zero_center=False, com=True): # tyll
        x_lig, x_rec = x
    ...
    def sample_p_zs_given_zt(self, s, t, zt_lig, xh0_pocket, ligand_mask,
        ...
        # if torch.isnan(mu_lig).any().item():
        #     breakpoint()
    ...
    def sample_p_zt_given_zs(self, zs_lig, xh0_pocket, ligand_mask, pocket_mask,
                             s, t, com=False):
        """Samples from zt ~ p(zt | zs). Used noising"""
        gamma_s = self.gamma(s)
        gamma_t = self.gamma(t)
        ...
        if com:
            zt_lig, xh0_pocket = self.sample_normal_zero_com(
                mu_lig, xh0_pocket, sigma_t_given_s, ligand_mask, pocket_mask, com)
            self.assert_mean_zero_with_mask(zt_lig, ligand_mask)
        else:
            zt_lig = self.sample_normal_zero_com(
                mu_lig, xh0_pocket, sigma_t_given_s, ligand_mask, pocket_mask, com)
