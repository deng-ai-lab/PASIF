...
@register_transform('set_lig_gen')
@register_mode_transform('set_lig_gen')
class SetLigandGen(object):
    ...
    def __call__(self, data):
        ...
        x = [get_index(e, h, a, self.mode) for e, h, a in zip(element_list, hybridization_list, aromatic_list)]
        x = torch.tensor(x)
        data.ligand['atom_type'] = F.one_hot(x, num_classes=self.num_types)
        data.ligand['lig_flag'] = torch.ones_like(data.ligand['element'], dtype=bool)
        
        return data

@register_transform('set_frag_gen')
@register_mode_transform('set_frag_gen')
class SetFragGen(object):
