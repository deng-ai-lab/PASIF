...
@register_transform('center_pos_specify')
class CenterPosSpecify(object):
    def __init__(self, center_flag, mask_flag=None):
        self.center_flag = center_flag
        self.mask_flag = mask_flag
    ...
    def __call__(self, data):
        ...
        data.protein.pos = data.protein.pos - data_center
        data.offtarget.pos = data.offtarget.pos - data_center
        data.protein.translation = data_center.expand(data.protein.pos.size(0), -1)
        data.offtarget.translation = data_center.expand(data.offtarget.pos.size(0), -1)
        if hasattr(data, 'ligand'):
            if hasattr(data.ligand, 'pos'):
                data.ligand.pos = data.ligand.pos - data_center
                data.ligand.translation = data_center.expand(data.ligand.pos.size(0), -1)
...
@register_transform('center_pos_admet')
class CenterPosADMET(object):
...
@register_transform('center_whole_pos')  # tyll
class CenterPosWhole(object):
    def __init__(self):
        pass 
...
@register_transform('center_whole_pos_spec')  # tyll
class CenterPosWholeSpecify(object):
    def __init__(self):
        pass 
