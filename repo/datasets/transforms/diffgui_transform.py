...
from repo.datasets.diffgui_data import ProteinLigandData, ProteinLigandDataSpecify
...
aromatic_idx = {f: i for i, f in enumerate(atom_families)}
...
class FeatureComplex(object):
    def __init__(self, mode="basic", sample=False, specify=False):
        super().__init__()
    ...
    def __call__(self, data: ProteinLigandData):
        ...
        data.ligand_bond_feat = F.one_hot(data.ligand_bond_type - 1, num_classes=len(bond_types))

        # build half edge
        if not self.sample:
            # data.ligand_num_atoms = data.ligand_element.shape[0]  # tyll
    ...
    def decode_output(self, pred_node, pred_pos, pred_halfedge, halfedge_index, ignore_H=False):
        """
        ...
        if ignore_H:
            top_1 = pred_atom.argmax(axis=-1)
            partitioned = np.argpartition(pred_atom, -2, axis=-1)
...
class FeatureComplexSpecify(object):
    def __init__(self, mode="basic", sample=False, specify=False):
        super().__init__()
        # H, C, N, O, Mg, P, S, Ca, Mn, Fe, Co, Ni, Cu, Zn, Se, Mo
        # self.protein_atomic_numbers = torch.LongTensor([1, 6, 7, 8, 12, 15, 16, 20, 25, 26, 27, 28, 29, 30, 34, 42])
    ...
    def __call__(self, data: ProteinLigandDataSpecify):
        data.protein_num_atoms = len(data.protein_element)
        data.off_num_atoms = len(data.off_element)
        element = data.protein_element.view(-1, 1) == self.protein_atomic_numbers.view(1, -1)
        amino_acid = F.one_hot(data.protein_atom_to_aa_type, num_classes=self.max_num_aa)
        is_backbone = data.protein_is_backbone.view(-1, 1).long()
        x = torch.cat([element, amino_acid, is_backbone], dim=-1)
        ...
        element = data.off_element.view(-1, 1) == self.protein_atomic_numbers.view(1, -1)
        amino_acid = F.one_hot(data.off_atom_to_aa_type, num_classes=self.max_num_aa)
        is_backbone = data.off_is_backbone.view(-1, 1).long()
        x = torch.cat([element, amino_acid, is_backbone], dim=-1)
        data.off_atom_feat = x
