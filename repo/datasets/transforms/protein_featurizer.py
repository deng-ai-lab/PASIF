...
@register_transform('featurize_offtarget_fa')
class FeaturizeOfftargetFullAtom(object):
    ...
    def __call__(self, data):
        data_prot = {}
        element = (data.offtarget.element.view(-1, 1) == self.atomic_numbers.view(1, -1)).float()
        amino_acid = data.offtarget.atom_to_aa_type
        is_backbone = data.offtarget.is_backbone.view(-1, 1).long()
        x = torch.cat([element, is_backbone], dim=-1)

        data_prot['atom_feature'] = x
        data_prot['aa_type'] = amino_acid
        data_prot['pos'] = data.offtarget.pos
        data_prot['element'] = data.offtarget.element
        data_prot['lig_flag'] = torch.zeros_like(data.offtarget.element, dtype=torch.bool)
        data_prot['atom_type'] = torch.tensor([get_index(e, h, a, 'basic') for e, h, a in zip(data.offtarget.element, 
                                                                                 torch.zeros_like(data.offtarget.element), 
                                                                                 torch.zeros_like(data.offtarget.element))])
        data_prot['alpha_carbon_indicator'] = torch.tensor([True if name =="CA" else False for name in data.offtarget['atom_name']])
        
        if hasattr(data.offtarget, 'contact'):
            data_prot['contact'] = data.offtarget['contact']
            data_prot['contact_idx'] = data.offtarget['contact_idx']

        data.offtarget = data_prot    
...
@register_transform('featurize_protein_res')
