...
def get_index(atom_num, hybridization, is_aromatic, mode):
    if mode == 'basic':
        if int(atom_num) in map_atom_type_only_to_index:
            return map_atom_type_only_to_index[int(atom_num)]
        else:
            print(int(atom_num))
            return map_atom_type_only_to_index[1]
        ...
        if (int(atom_num), bool(is_aromatic)) in map_atom_type_diffgui_to_index:
            return map_atom_type_diffgui_to_index[int(atom_num), bool(is_aromatic)]
        else:
            print(int(atom_num), bool(is_aromatic))
            return map_atom_type_diffgui_to_index[(1, False)]
    else:
