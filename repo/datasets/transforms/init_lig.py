...
@register_transform('assign_molsize_around')
class AssignMolSizeAround(object):

    def __init__(self, mean=21, std=5):
        super().__init__()
        self.mean = mean
        self.std = std

    def __call__(self, data):
        num_atoms_lig = self.mean + random.randint(-self.std, self.std)
        data.ligand.atom_type = torch.zeros(num_atoms_lig, dtype=torch.long)
        data.ligand.lig_flag = torch.ones(num_atoms_lig, dtype=torch.bool)
        data.ligand.pos = torch.zeros(num_atoms_lig, 3, dtype=torch.float)
        data.ligand.element = torch.zeros(num_atoms_lig, dtype=torch.long)
...
class AssignGenSize(object):
    ...
    def __call__(self, data):
        ...
        # num_atoms_lig = max(num_atoms_lig, int(0.8*data.ligand.ori_num))  # tyll
        # num_atoms_lig = min(num_atoms_lig, int(1.2*data.ligand.ori_num))  # tyll
...
@register_transform('assign_leadoptsize')
class AssignLeadOptSize(object):

    def __init__(self, mode, distribution='prior_distcond'):
        super().__init__()
        self.distribution = distribution
        self.mode = mode
    ...
    def __call__(self, data):
        ...
        data.ligand['ori_num'] = data.ligand['element'].shape[0]  # tyll
        # data.ligand['atom_type'] = x
        # data.ligand['element'] = data.ligand['element']
...
class AssignGenType(object):
    ...
    def __init__(self, distribution='uniform', mode='add_aromatic') -> None:
        ...
        if self.mode == 'add_aromatic':
            ...
            self.num_types = len(map_atom_type_diffgui_to_index)
...
class SaveEntry(object):
    ...
    def __call__(self, data) -> torch.Any:
        data.ligand.entry = data.entry[1]
        data.protein.entry = data.entry[0]
        return data
