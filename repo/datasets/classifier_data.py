...
from repo.datasets.transforms._base import TRANSFORM_DICT
from repo.datasets.parsers import torchify_dict
from repo.datasets.parsers.molecule_parser import parse_smiles
from repo.datasets.transforms.classifier_trans import MergeKeys
from repo.datasets.transforms.classifier_trans import CenterPos
from repo.datasets.transforms.classifier_trans import NormalizeVina, AddMolProp
from repo.datasets.transforms.classifier_trans import FeaturizeProteinFullAtom, FeaturizeProteinResidue
from repo.datasets.transforms.classifier_trans import FeaturizeLigandFullAtom
...
def get_dataset(config, mode):
    name = config.name
    root = config.path
    # transform_list = get_transform_list(config.transform)
    if mode == 'diffgui':
        transform_list = [FeaturizeProteinResidue(), FeaturizeLigandFullAtom(mode=mode),
                        CenterPos(), NormalizeVina(), AddMolProp(), MergeKeys(keys=['protein', 'ligand'])]
    else:
        transform_list = [FeaturizeProteinFullAtom(), FeaturizeLigandFullAtom(mode=mode),
                        CenterPos(), NormalizeVina(), AddMolProp(), MergeKeys(keys=['protein', 'ligand'])]
    # transform_list.extend([NormalizeVina(), AddMolProp()])
    transform = Compose(transform_list)
    if name == 'classifier_pl':
        dataset = PocketLigandPairDataset(root, transform=transform)
    else:
        raise NotImplementedError('Unknown dataset: %s' % name)
    
    if config.get('split_path', None) is not None:
        split = torch.load(config.split_path)
...
class ADMETDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        smiles = self.data_df.iloc[index, 0]
        label = self.data_df.iloc[index, 1]
        sdf_path = self.data_df.iloc[index, 3]
        sdf_path = self.csv_path.split('/')[:-1] + ['sdf', sdf_path]
        sdf_path = '/'.join(sdf_path)
        lig_data = parse_sdf_file(sdf_path)
        # lig_data = parse_smiles(smiles=smiles)
        data = EasyDict({'ligand': torchify_dict(lig_data)})
        data['label'] = label
        if self.transform is not None:
            data = self.transform(data)
        return data