...
map_atom_type_diffgui_to_index = {
    (1, False): 0,
    (5, False): 1,
    (6, False): 2,
    (6, True): 3,
...
map_index_to_atom_type_diffgui = {v: k for k, v in map_atom_type_diffgui_to_index.items()}
...
def get_atomic_number_from_index(index, mode):
    if mode == 'basic':
        ...
        atomic_number = [map_atom_type_diffgui_to_index[i][0] for i in index.tolist()]
    else:
        raise ValueError
...
def is_aromatic_from_index(index, mode):
    if mode == 'add_aromatic':
        ...
        is_aromatic = [map_index_to_atom_type_diffgui[i][2] for i in index.tolist()]
    elif mode == 'full':
...
map_atom_symbol_to_atomic_number= {'H':1, 'B': 5, 'C':6, 'N':7, 'O':8, 'F':9, 'P': 15, 'S':16, 'Cl': 17, 'Br': 35, 'I': 53}
