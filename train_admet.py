...
from repo.datasets.classifier_data import ADMETDataset
...
from repo.datasets.transforms.translation import CenterPosADMET
from repo.datasets.transforms.classifier_trans import NormalizeLabel
from repo.datasets.transforms.molecule_featurizer import FeaturizeLigandFullAtom
...
if __name__ == '__main__':
    ...
    transform_list = [FeaturizeLigandFullAtom(mode=mode), CenterPosADMET(center_flag='ligand'), 
                      NormalizeLabel(prop=args.prop), MergeKeys(keys=['ligand'])]
    transform = Compose(transform_list)
    train_dataset = ADMETDataset(csv_path=f'./data/classifier/{args.prop}/filter_{args.prop}_train.csv',
                                 transform=transform)
    val_dataset   = ADMETDataset(csv_path=f'./data/classifier/{args.prop}/filter_{args.prop}_test.csv',
                                 transform=transform)
    ...
    model = PropPredictor(cfg=config.model, model_type=model_type).to(args.device)
    ...
    def train(it):
        ...
        # Forward pass
        loss_dict, _ = model.get_loss(prop=batch['ligand_label'], 
                                      x_rec=None,
    ...
    def validate(it, evaluator):
        ...
        with torch.no_grad():
            ...
            for i, batch in enumerate(tqdm(val_loader, desc='Validate', dynamic_ncols=True)):
                ...
                # Forward pass
                loss_dict, results = model.get_loss(prop=batch['ligand_label'], 
                                      x_rec=None,
                                      x_lig=batch['ligand_pos'], 
                                      h_rec=None,
                                      h_lig=batch['ligand_atom_type'],
